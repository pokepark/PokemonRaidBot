...
//Check is 12 digit number
$friend_code = preg_replace('/[^[:digit:]]/', '',(substr($update['message']['text'], 7)));

if (strlen($friend_code) == 12) {
	...
	if ($update['message']['chat']['type'] == 'private' || $update['callback_query']['message']['chat']['type'] == 'private') {
		...
		sendMessage($update['message']['chat']['id'], getTranslation('friend_code_update_fail'));
	...
	// Write to log.
	debug_log('Invalid friend code: ' . $friend_code);
	
	//Invalid code
	sendMessage($update['message']['chat']['id'], getTranslation('friend_code_invalid'));
