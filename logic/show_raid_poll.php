...
function show_raid_poll($raid, $inline = false)
{
  ...
  if($raid['event_pokemon_title'] != 0 or $raid['event_pokemon_title'] == NULL) {
    ...
    $msg = raid_poll_message($msg, $title . ': <b>' . get_local_pokemon_name($raid_pokemon_id, $raid['pokemon_form'], true) . ' ' . ($raid['shadow'] ? ' ' . getPublicTranslation('shadow') : '') . '</b>', true);
  ...
  $att_array = $cnt_array = $done_array = $cancel_array = [];
  $cnt_all = $cnt_remote = $cnt_want_invite = $cnt_extra_alien = $cnt_latewait = $cnt_cancel = $cnt_done = $cnt_can_invite = 0;
