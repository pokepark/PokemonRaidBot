...
function show_raid_poll_small($raid)
{
  ...
  if(!empty($raid['pokemon'])) {
    $msg .= '<b>' . get_local_pokemon_name($raid['pokemon'], $raid['pokemon_form']) . (isset($raid['shadow']) && $raid['shadow'] && !in_array($raid['pokemon'], EGGS) ? ' ' . getPublicTranslation('pokemon_form_shadow') : '') . '</b> ' . CR;
  }
